...
#### [Game:End] - 结束当前牌局
-   **功能**: 宣告游戏结束。此指令不负责分配奖励，奖励必须通过后续的 `[Event:Modify]` 指令处理。
-   **data JSON 结构**:
```json
{
    "result": "win" | "lose" | "escape" | "boss_win",
    "reason": "游戏结束原因的简短描述"
}
```
- **result说明**:
...
-   `visibility` **说明**:
    -   `"public"`: 对所有人可见。
    -   `"owner"`: 仅对卡牌拥有者可见 (玩家手牌的标准设置)。
    -   `"hidden"`: 对所有人显示为牌背 (对手秘密手牌的必需设置)。
